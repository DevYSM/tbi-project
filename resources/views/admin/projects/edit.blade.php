    ...
    <div class="row pt-5">
        <div class="col-12">
            ...
            <div class="row justify-content-center">
                <div class="col-xl-12 order-xl-1">
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            <form method="post" action="{{ route('projects.editprojects', $projects->id) }}"
                                ...
                                <div class="row">
                                    <div class="col-12 col-sm-4">
                                        <div class="form-group{{ $errors->has('main_title') ? ' has-danger' : '' }}">
                                            ...
                                            <input type="text" name="main_title" id="main_title"
                                                ...
                                                value="@if(!empty($projects)){{ $projects->main_title }}@else{{ old('main_title') }}@endif"
                                                autofocus>
                                    ...
                                    {{-- {{ dd($projects->project_tags) }} --}}
                                    <div class="col-12 col-sm-4">
                                        <div class="form-group">
                                            <label>Tags</label>

                                            @php
                                            $projectTagIds= $project_tags->pluck('id')->toArray();
                                            @endphp

                                            {{-- {{ dd(count($project_tags)) }} --}}
                                            <div class="select2-primary">
                                                <select name="tags[]" class="select2" multiple="multiple"
                                                    ...
                                                    @foreach ($tags as $tag)
                                                    <option value="{{ $tag->id }}"
                                                        {{ in_array($tag->id, $projectTagIds)  ? 'selected' : ''}}>
                                                        {{ $tag->title }}
                                                    </option>
                                                    @endforeach
                                    ...
                                    {{-- {{ dd($projects->project_technologies) }} --}}
                                    <div class="col-12 col-sm-4">
                                        <div class="form-group">
                                            @php
                                            $projectTechIds= $project_technologies->pluck('id')->toArray();
                                            @endphp
                                            ...
                                            <div class="select2-primary">
                                                <select name="technologies[]" class="select2" multiple="multiple"
                                                    ...
                                                    <option value="{{ $tech->id }}"
                                                        {{ in_array($tech->id, $projectTechIds)  ? 'selected' : ''}}>
                                                        {{ $tech->title }}
                                                    </option>
                                                    @endforeach
                                    ...
                                    <div class="col-12 col-sm-12 ">
                                        <div class="form-group{{ $errors->has('photo') ? ' has-danger' : '' }}">
                                            ...
                                            <input type="file" name="photo" class="dropify" data-max-file-size="2M"
                                                ...
                                                data-default-file="{{ url('/'.$projects->photo) }}" />
                                            @if ($errors->has('photo'))
            ...
            <div class="card">
                ...
                @php
                $langCodeArray = $data->pluck('lang_code')->toArray();
                @endphp
                {{ dd($data) }}
                {{-- {{ dd($data) }} --}}
                <div class="card-body">
                    <div class="tab-content">
                        ...
                        <div class="tab-pane {{ $loop->index == 0 ? 'active' : '' }}" id="tab_{{  $lan->code }}">
                            ...
                            <form method="post" action="{{ route('projects.trans_update', $data[$x]->id) }}"
                                autocomplete="off" enctype="multipart/form-data">
                                @csrf
                                @else
