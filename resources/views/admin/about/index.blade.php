    ...
    <div class="row pt-5">
        <div class="col-12">
            ...
            <div class="row justify-content-center">
                <div class="col-xl-12 order-xl-1">
                    <div class="card card-info">
                        ...
                        <div class="card-body">
                            @if (!empty($mainTitle))
                            <form method="post" action="{{ route('about.edittitle', $mainTitle->id) }}"
                                autocomplete="off" enctype="multipart/form-data">
                                @else
                                <form method="post" action="{{ route('about.storetitle') }}" autocomplete="off"
                        ...
                        <div class="tab-pane {{ $loop->index == 0 ? 'active' : '' }}" id="tab_{{  $lan->code }}">
                            ...
                            @if(count($data)>0)
                                ...
                                @if ( $data[$x]->lang_code == $lan->code )
                                    <form method="post" action="{{ route('about.trans_update', $data[$x]->id) }}"
                                    autocomplete="off" enctype="multipart/form-data">
                                    @csrf
                                @else
                            ...
                            {{-- End Check Count of Data Translated To Create Update Form --}}
                                    ...
                                    <div class="row">
                                        ...
                                        <input type="hidden" name="about_id" value="{{ $mainTitle->id }}">
                                        {{-- Start Grid --}}
                                        ...
                                        <div class="col-12 col-sm-6 ">
                                            ...
                                            <div
                                                ...
                                                <label class="form-control-label" for="description">{{ $lan->title }}
                                        ...
                                        <div class="col-12 col-sm-12 ">
                                            <div class="text-center">
                                                @if(count($data)>0)
                                                @if ( $data[$x]->lang_code == $lan->code)
                                                <button type="submit" class="btn btn-danger mt-4  w-25">{{ __('Edit') }}
                                                    <i class="fas fa-check ml-2 fa-sm"></i></button>
                                                @else
                                                <button type="submit" class="btn btn-info mt-4  w-25">{{ __('Save') }}
                                                    <i class="fas fa-check ml-2 fa-sm"></i></button>
                                                @endif
                                                @else 
                                                <button type="submit" class="btn btn-info mt-4  w-25">{{ __('Save') }}
            ...
            <div class="alert alert-danger text-center">
                <h4 class="m-0">You must add main title first to add other language.</h4>
            </div>
            @endif
            <!-- ./card -->
